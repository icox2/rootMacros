...
void noriCrateTimingAnalyzer(){
  ofstream fout("crate1opt.dat");
  TH1D *tdiffHist[9][9];
  for(int i=0;i<9;i++){
    for(int j=0;j<9;j++){
      char c[4] = "td";
      ...
      char b = ('0'+j);
      c[2] = a; c[3] = b;
      tdiffHist[i][j] = new TH1D(c,c,4000.,-100.,100.);
    }
  }
  TFile *_file0 = TFile::Open("crate1_DD.root");
...
while(singe.Next()){
  ...
  for(auto itC = rd.begin(); itC!=rd.end();itC++){
      ...
      else if(channel==15 && module==1){ //external trig
          itwo++;
          ttwo = *trace;
          double hold =0;
  ...
  if((ione>0 && itwo>0) && (energyone>40000 && energytwo>40000)){
  ...
  for(int it=0;it<9;it++){
    for(int jt=0;jt<9;jt++){
      ...
      binMax = binMax/20.-100.;
      TF1 *g = new TF1("g","gaus",binMax-5,binMax+5);
      g->SetParameters(max,binMax,0.4);
      tdiffHist[it][jt]->Fit(g,"RNQSW");
      //sleep(2);
      if((g->GetParameter(2))>1.){
        cout<<"Error in the fit for "<<it<<" "<<jt<<" "<<binMax<<" "<<g->GetParameter(1)<<endl;
      }
      else if((g->GetParameter(2))<minTDiff){
        minf1 = it; minf2=jt;
