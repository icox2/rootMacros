...
void mtasSubInnerSum(TTree *OutputTree){
  ...
  //OutputTree->Draw("mtas_sum>>es","dT>0 && abs(dT)<400 && abs(mtas_t)<75","");
  //OutputTree->Draw("mtas_sum>>eb","dT<0 && abs(dT)<400 && abs(mtas_t)<75","");
  OutputTree->Draw("mtas_inner+mtas_central>>es","betaEnergySum<7000 && dT>0 && abs(mtas_t)<75","");
  OutputTree->Draw("mtas_inner+mtas_central>>eb","betaEnergySum<7000 && dT<0 && abs(mtas_t)<75","");
  es->Sumw2();
  eb->Sumw2();
  es->Add(eb,-1/bkgRatio);
  int rebin = 1;
  es->Rebin(rebin);
  TString histName = fname+" MTAS InnerSum Energy Spectrum;Energy (keV);Counts/"+to_string(2*rebin)+"keV";
