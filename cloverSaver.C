...
#include <iostream>
{
  ...
  cout<<fName<<endl;
  OutputTree->Draw("cloverAAB_E>>ces(4900,10,5000)", "abs(dT)<500 && abs(cloverAAB_Twc)<100 && dT>0 && dr<0.1 ","colz");
  OutputTree->Draw("cloverAAB_E>>cesb(4900,10,5000)","abs(dT)<500 && abs(cloverAAB_Twc)<100 && dT<0 && dr<0.1","colz");
  ces->Sumw2();
  cesb->Sumw2();
  ces->Add(cesb,-1);
  ces->Draw("hist");
  ...
  TH1F *h_gammaAAB_betaprompt = (TH1F*)ces->Clone();
  h_gammaAAB_betaprompt->SetTitle("h_gammaAAB_betaprompt");
  h_gammaAAB_betaprompt->SetName("h_gammaAAB_betaprompt");
  TString fileName = "vandleHistograms/"+fName+"_vandle_hists.root";
  TFile *_file2 = TFile::Open(fileName,"UPDATE");
  h_gammaAAB_betaprompt->Write();
  _file2->Close();
}
