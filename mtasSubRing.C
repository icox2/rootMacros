...
void mtasSubRing(TTree *OutputTree, int ringNum=1){
  ...
  TString gate = "betaEnergySum<7000 && mtas_ringnum=="+to_string(ringNum); 
  OutputTree->Draw("mtas_E>>es",gate+" && dT>0","");
  OutputTree->Draw("mtas_E>>eb",gate+" && dT<0","");
  es->Sumw2();
  eb->Sumw2();
  es->Add(eb,-1);
  int rebin = 1;
  es->Rebin(rebin);
  TString histName = fname+" MTAS;Energy (keV);Counts/"+to_string(2*rebin)+"keV";
